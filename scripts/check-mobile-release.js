...
// The .xcworkspace is a CocoaPods artifact created by `pod install` (macOS only), so it is
// not committed — but run-ios-release-build.js archives with `xcodebuild -workspace`, so a
// missing workspace still blocks the release build. Keep it fatal, with a message that
// says how to produce it rather than implying the checkout is broken.
if (!fs.existsSync(iosProject)) {
  failures.push(
  ...
  failures.push(
    'iOS workspace is missing at apps/mobile/ios/TopoSonics.xcworkspace; it is generated by ' +
      'CocoaPods, so run `npx pod-install` (or `cd apps/mobile/ios && pod install`) on macOS'
  );
}
